...
@Mixin()
public class Property_salesAreaFoodAndBeveragesTotal {

    final private Property property;

    public Property_salesAreaFoodAndBeveragesTotal(Property property) {
        this.property = property;
    }

    @Action(semantics = SemanticsOf.SAFE)
    @ActionLayout(contributed = Contributed.AS_ASSOCIATION)
    public BigDecimal salesAreaFoodAndBeveragesTotal() {
        return occupancyRepository.findByProperty(property).stream()
                ...
                .map(sal->sal.getFoodAndBeveragesArea())
